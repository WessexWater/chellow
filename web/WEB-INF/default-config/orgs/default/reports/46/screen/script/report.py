...
invoice_element = invoice.getXML(XmlTree('batch', XmlTree('service', XmlTree('provider', XmlTree('organization')))).put('bill', XmlTree('account', XmlTree('provider', XmlTree('organization')))), doc)
...
for register_read in Hiber.session().createQuery("from RegisterRead read where read.invoice = :invoice order by read.units.int, read.tpr.code").setEntity('invoice', invoice).list():
    invoice_element.appendChild(register_read.getXML(XmlTree('mpan', XmlTree('supplyGeneration').put('mpanCore')).put('tpr'), doc));