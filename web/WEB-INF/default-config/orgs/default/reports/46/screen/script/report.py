...
if not invoice.getBatch().getContract().getOrganization().equals(organization):
    raise UserException("Such an invoice doesn't exist in this organization")
invoice_element = invoice.toXml(doc, XmlTree('batch', XmlTree('contract', XmlTree('provider').put('organization'))).put('bill', XmlTree('account', XmlTree('provider', XmlTree('organization')))))
