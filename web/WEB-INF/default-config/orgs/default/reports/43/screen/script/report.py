...
bill_element = bill.toXml(doc, XmlTree('account', XmlTree('provider', XmlTree('organization'))))
...
for invoice in Hiber.session().createQuery("from Invoice invoice where invoice.bill = :bill order by invoice.startDate.date").setEntity('bill', bill).list():
    bill_element.appendChild(invoice.toXml(doc, XmlTree('batch', XmlTree('service'))))