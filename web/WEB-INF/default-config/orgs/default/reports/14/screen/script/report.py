...
stmt = con.prepareStatement("select hh_datum.value, hh_datum.end_date, hh_datum.status, channel.is_import, supply.name, source.code from hh_datum hh_datum, channel channel, supply supply, site_supply site_supply, site site, source source where hh_datum.channel_id = channel.id and channel.supply_id = supply.id and supply.id = site_supply.supply_id and site_supply.site_id = site.id and supply.source_id = source.id and site.code = ? and channel.is_kwh is true and hh_datum.end_date >= ? and hh_datum.end_date <= ? and source.code != 'sub' order by hh_datum.end_date", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY, ResultSet.CLOSE_CURSORS_AT_COMMIT)
