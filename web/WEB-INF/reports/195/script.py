...
if sys.platform.startswith('java'):
    ...
    Contract, Era, Supply, Source = db.Contract, db.Era, db.Supply, db.Source
    ...
    def content():
        ...
        try:
            ...
            try:
                ...
                raise UserException(
                    "The property " + ECOES_USER_NAME_KEY +
            ...
            if proxy_host is not None:
                ...
                if proxy_port is None:
                    raise UserException(
                        "The property " + PROXY_HOST_KEY +
                        " is set, but the property " + PROXY_PORT_KEY +
                        " is not.")
            ...
            http_post.setEntity(
                UrlEncodedFormEntity(
                    [
                        BasicNameValuePair("username", user_name),
                        BasicNameValuePair("password", password),
                        BasicNameValuePair('beenHereBefore', '1'),
                        BasicNameValuePair('forceLogout', '1')],
                    HTTP.UTF_8))
            ...
            mpans = sess.query(Era.imp_mpan_core).join(Supply, Source). \
                join(Supply.dno_contract).filter(
                    ...
                    Era.imp_mpan_core).all() + \
                sess.query(Era.exp_mpan_core).join(Supply, Source) \
                .join(Supply.dno_contract).filter(
                    Contract.name != '99', Era.finish_date == null(),
                    Source.code != '3rd-party',
                    Era.exp_mpan_core != null()).distinct().order_by(
                    Era.exp_mpan_core).all()

            mpans = [v[0] for v in mpans]

            http_get = HttpGet(
                'http://www.ecoes.co.uk/saveportfolioMpans.asp?guid=' + guid)

            yield "MPAN Core,MPAN Core No Spaces,ECOES PC,Chellow PC, " \
                "ECOES MTC,Chellow MTC,ECOES LLFC,Chellow LLFC,ECOES SSC, " \
                "Chellow SSC,ECOES Supplier, Chellow Supplier,ECOES DC, " \
                "Chellow DC,ECOES MOP,Chellow MOP,ECOES GSP Group, " \
                "Chellow GSP Group,ECOES MSN, Chellow MSN,ECOES Meter Type, " \
            ...
            for values in parser:
                ...
                ecoes_titles = [
                    ...
                    'address-line-9', 'post-code', 'supplier',
                    'registration-from', 'mtc', 'mtc-date', 'llfc',
                    'llfc-from', 'pc', 'ssc', 'measurement-class',
                    'energisation-status', 'da', 'dc', 'mop',
                    'mop-appoint-date', 'gsp-group', 'gsp-effective-from',
                    'dno', 'msn', 'meter-install-date', 'meter-type', 'map-id']
                ...
                if disconnected and current_chell:
                    problem += \
                        "Disconnected in ECOES, but current in Chellow. "
