...
try:
    ...
    for i in range(months):
        ...
        for supply in supplies.filter(
                ...
                or_(Era.finish_date == None, Era.finish_date >= month_start)):
            ...
            site = sess.query(Site).join(SiteEra).filter(
                SiteEra.era_id == era.id, SiteEra.is_physical == True).one()
            
            values = {
                'supply-name': supply.name, 'source-code': source_code,
                'generator-type': generator_type,
                'month': hh_format(month_finish), 'pc-code': era.pc.code,
                'msn': era.msn, 'site-code': site.code, 'site-name': site.name,
                'metering-type': metering_type, 'problem': ''}
            ...
            for is_import, pol_name in [(True, 'import'), (False, 'export')]:
                ...
                if metering_type in ['hh', 'amr']:
                    est_kwh = sess.query(HhDatum.value).join(Channel) \
                        .join(Era).filter(
                            HhDatum.status == 'E', Era.supply_id == supply.id,
            ...
            for bill in sess.query(Bill).filter(
                    Bill.supply_id==supply.id, Bill.start_date<=month_finish,
            ...
            for era in eras:
                ...
                if source_code in ['net', 'gen-net', '3rd-party']:
                    ...
                    if import_vb_function is None:
                        values['problem'] += "Can't find the virtual_bill " \
                            "function in the supplier contract. "
                    else:
                    ...
                    if mop_func is None:
                        values['problem'] += " MOP virtual_bill function " \
                            "can't be found."
                    else:
                        mop_func(supply_source)
                        values['metered-import-net-gbp'] += \
                            supply_source.mop_bill['net-gbp']
