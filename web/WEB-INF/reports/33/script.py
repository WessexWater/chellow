...
def content():
    ...
    try:
        ...
        eras = sess.query(Era).filter(
            Era.start_date < date,
            or_(Era.finish_date == None, Era.finish_date >= date)).order_by(
            Era.supply_id)
    ...
    """, params={'supply_id': supply.id, 'year_start': year_start, 'year_finish': date, 'is_import': is_import}))
            ...
            yield ','.join(
                ...
                for value in [
                    ...
                    None if era.imp_supplier_contract is None else era.imp_supplier_contract.name,
                    ...
                    None if era.exp_supplier_contract is None else era.exp_supplier_contract.name,
