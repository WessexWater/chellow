...
try:
    sess = db.session()
    pw.println("Site Id, Site Name, Associated Site Ids, Sources, Generator Types, Date, Imported kWh, Displaced kWh, Exported kWh, Used kWh, Parasitic kWh, Generated kWh,Meter Type")
    pw.flush()
    ...
    for site in sites:
        ...
        for group in site.groups(sess, start_date, finish_date, True):
            ...
            for hh in group.hh_data(sess):
                pw.println(','.join('"' + str(v) + '"' for v in (site.code, site.name, assoc_str, sources_str, generators_str, hh['start_date'].strftime("%Y-%m-%d %H:%M"), hh['imp_net'], hh['displaced'], hh['exp_net'], hh['used'], hh['exp_gen'], hh['imp_gen'], metering_type)))
                pw.flush()
    pw.close()
finally:
    if sess is not None:
