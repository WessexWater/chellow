...
def test_do_get_as_html(mocker, sess, client):
    ...
    mop_contract = mop_party.insert_contract(
        sess, "Fusion Mop Contract", "", {}, vf, None, {}
    )
...
def test_do_get_as_html_with_issue(mocker, sess, client):
    vf = to_utc(ct_datetime(2000, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    market_role_Z = MarketRole.get_by_code(sess, "Z")
    participant = Participant.insert(sess, "CALB", "AK Industries")
    ...
    era = supply.eras[0]
    era.insert_channel(sess, True, "ACTIVE")
    issue = imp_supplier_contract.insert_issue(
        sess, to_utc(ct_datetime(1997, 1, 1)), {}
    )
    ...
    response = client.get("/reports/channel_snags", query_string=query_string)
    for snag in sess.scalars(select(Snag)):
        print(
            snag.id,
            snag,
            "channel",
            snag.channel.id,
            "contract id",
            snag.channel.era.dc_contract.id,
        )
    match(response, 200, r"<td>Missing</td>")
...
def test_content_days_long_hidden(mocker, sess):
    vf = to_utc(ct_datetime(2000, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    market_role_Z = MarketRole.insert(sess, "Z", "Non-core")
    participant = Participant.insert(sess, "CALB", "AK Industries")
