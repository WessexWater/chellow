...
def test_report_post(sess, client, mocker):
...
"""
    ...
    mock_content = mocker.patch("chellow.reports.report_149.content")

    response = client.post("/reports/149", data=data)

    match(response, 303)

    user = User.get_by_email_address(sess, "admin@example.com")

    expected_scenario_props = {
    ...
    mock_content.assert_called_with(expected_scenario_props, user.id)
...
def test_process_mpan_core(sess, mocker):
    valid_from = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
