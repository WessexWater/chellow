...
from chellow.reports.report_g_supply_virtual_bill import content
...
"""
    ...
    mocker.patch(
        "chellow.reports.report_g_supply_virtual_bill.open", return_value=mock_file
    )
    mocker.patch(
        "chellow.reports.report_g_supply_virtual_bill.chellow.dloads.make_names",
        return_value=("a", "b"),
    )
    mocker.patch("chellow.reports.report_g_supply_virtual_bill.os.rename")

    user = mocker.Mock()
    g_supply_id = g_supply.id
    file_name = "a.csv"
    start_date = to_utc(ct_datetime(2020, 9, 1))
    finish_date = to_utc(ct_datetime(2020, 9, 1))

    content(g_supply_id, file_name, start_date, finish_date, user.id)
