...
def test_channel_add_post_export_related(sess, client):
    vf = to_utc(ct_datetime(2000, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    ...
    response = client.post(f"/e/eras/{era.id}/add_channel", data=data)
    match(response, 303)
    response = client.get("/e/channels/1")
    match(response, 200, r"Channel Export\s*ACTIVE")


def test_channel_snags_get(sess, client):
    ...
    sess.commit()
    query_string = {"dc_contract_id": "", "days_hidden": 5}

    response = client.get("/e/channel_snags", query_string=query_string)

    regex = []
    match(response, 200, *regex)
...
def test_dc_batch_upload_file_get(sess, client):
    ...
    market_role_C = MarketRole.insert(sess, "C", "DC")
    dc_party = participant.insert_party(
        sess, market_role_C, "Fusion Dc Ltd", vf, None, None
    )
    dc_contract = dc_party.insert_contract(sess, "Fusion", "", {}, vf, None, {})
...
def test_dc_contracts_get(sess, client):
    response = client.get("/e/dc_contracts")
...
def test_dc_contract_edit_post_error(sess, client):
    ...
    contract = dc_party.insert_contract(
        sess, "Lowri Beck", "", {}, utc_datetime(2000, 1, 1), None, {}
    )
    sess.commit()
...
def test_dc_contracts_hh_imports_get(sess, client):
    ...
    contract = dc_party.insert_contract(sess, "DC 2000", "", {}, vf, None, {})
    sess.commit()
...
def test_dc_auto_importer_get(sess, client):
    ...
    contract = dc_party.insert_contract(
        sess, "DC 2000", "", {}, utc_datetime(2000, 1, 1), None, {}
    )
    sess.commit()
...
def test_dc_contract_edit_delete(sess, client):
    ...
    market_role_C = MarketRole.insert(sess, "C", "DC")
    participant = Participant.insert(sess, "CALB", "AK Industries")
    dc_party = participant.insert_party(sess, market_role_C, "DC Ltd.", vf, None, None)
    contract = dc_party.insert_contract(sess, "DC 2000", "", {}, vf, None, {})
    sess.commit()
...
def test_dc_contract_edit_post(sess, client):
    ...
    properties = {"sora": "ai"}
    contract = party.insert_contract(sess, "DC 2000", "", properties, vf, None, {})
    sess.commit()
...
def test_dc_bill_get(sess, client):
    ...
    response = client.get(f"/e/dc_bills/{bill.id}")
    patterns = []
...
def test_era_supplier_bill_add_get(sess, client):
    vf = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    ...
    response = client.get(f"/e/eras/{era.id}/add_supplier_bill")

    patterns = []
    match(response, 200, *patterns)


def test_era_supplier_bill_add_post(sess, client):
    vf = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    ...
    data = {
        "batch_reference": "7743",
        "batch_description": "April 2026",
        "date_created_year": "2026",
        "date_created_month": "04",
        "date_created_day": "07",
        "date_created_hour": "15",
        "date_created_minute": "31",
        "date_created_second": "36",
        "date_created_microsecond": "4771",
        "account": "hrghj88",
        "bill_reference": "74hjkgjk",
        "issue_year": "2020",
        "issue_month": "02",
        "issue_day": "10",
        "issue_hour": "00",
    ...
    response = client.post(f"/e/eras/{era.id}/add_supplier_bill", data=data)
...
def test_dno_rate_script_edit_post(sess, client):
    ...
    dno_contract = dno_party.insert_contract(
        sess, dno_party.dno_code, "", {}, utc_datetime(2000, 1, 1), None, {}
    )
...
def test_era_edit_delete_fail(client, sess):
    vf = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    ...
    response = client.delete(f"/e/eras/{era.id}/edit")
...
def test_mop_batch_add_post(sess, client):
    vf = to_utc(ct_datetime(1996, 1, 1))
    participant = Participant.insert(sess, "hhak", "AK Industries")
    market_role_M = MarketRole.insert(sess, "M", "Mop")
    mop_party = participant.insert_party(
    ...
    data = {
        "contract_id": mop_contract.id,
        "reference": "kauegrhke",
        "description": "ytehrglkl",
        "date_created_year": "2020",
        "date_created_month": "01",
        "date_created_day": "01",
        "date_created_hour": "00",
        "date_created_minute": "01",
        "date_created_second": "53",
        "date_created_microsecond": "000001",
    }
...
def test_mop_batch_file_get(sess, client):
    ...
    contract = mop_party.insert_contract(sess, "Fusion", "", {}, vf, None, {})
    batch = contract.insert_batch(sess, "b1", "batch 1", vf)
    batch_file = batch.insert_file(sess, "bills.csv", b"a bill", "csv")
    sess.commit()
...
def test_mop_rate_script_get(sess, client):
    ...
    mop_contract = mop_party.insert_contract(sess, "Fusion MOP", "", {}, vf, None, {})
    mop_rate_script = mop_contract.rate_scripts[0]
    sess.commit()

    response = client.get(f"/e/mop_rate_scripts/{mop_rate_script.id}")
...
def test_site_add_e_supply_form_get(client, sess):
    ...
    mop_party.insert_contract(sess, "Fusion", "", {}, vf, None, {})
    dc_party.insert_contract(sess, "Fusion DC 2000", "", {}, vf, None, {})
    insert_cops(sess)
    insert_comms(sess)
    insert_dtc_meter_types(sess)
    supplier_party.insert_contract(sess, "Fusion Supplier 2000", "", {}, vf, None, {})
    dno_party = participant.insert_party(sess, market_role_R, "WPD", vf, None, "22")
...
def test_supplier_bill_get(sess, client):
    vf = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "22488", "Water Works")
    insert_sources(sess)
    source = Source.get_by_code(sess, "grid")
    ...
    comm = Comm.get_by_code(sess, "GSM")
    imp_supplier_contract = supplier_party.insert_contract(
        sess, "Fusion Supplier 2000", "", {}, utc_datetime(2000, 1, 1), None, {}
    )
    dno = participant.insert_party(sess, market_role_R, "WPD", vf, None, "22")
    meter_type = MeterType.insert(sess, "C5", "COP 1-5", vf, None)
    meter_payment_type = MeterPaymentType.insert(sess, "CR", "Credit", vf, None)
    ...
    bill.insert_element(
        ...
        to_utc(ct_datetime(2025, 1, 31, 23, 30)),
        Decimal("21.59"),
        {},
    )
    sess.commit()
    ...
    patterns = [
        ...
        r"<tbody>\s*"
        r"<tr>\s*"
        r'<td>\[<a href="/e/supplier_elements/1/edit">edit</a>\]</td>\s*'
        r"<td>nrg</td>\s*"
...
def test_supplier_bill_get_zish_error(sess, client):
    vf = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "22488", "Water Works")
    insert_sources(sess)
    source = Source.get_by_code(sess, "grid")
...
def test_supplier_bill_add_get(sess, client):
    vf = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "22488", "Water Works")
    insert_sources(sess)
    source = Source.get_by_code(sess, "grid")
    ...
    response = client.get(
        f"/e/supplier_batches/{batch.id}/add_bill", query_string=query_string
    )
    match(response, 200)
...
def test_supplier_contracts_add_post(sess, client):
    ...
    market_role_X = MarketRole.insert(sess, "X", "Supplier")
    participant = Participant.insert(sess, "CALB", "AK Industries")
    party = participant.insert_party(sess, market_role_X, "Fusion Ltc", vf, None, None)
    sess.commit()
...
def test_supplier_contract_get(client, sess):
    ...
    party.insert_contract(
        sess,
        "Fusion Supplier 2000",
        "",
        {},
        to_utc(ct_datetime(2000, 1, 3)),
        None,
        {},
    )
    sess.commit()
...
def test_supplier_issues_get(sess, client):
    vf = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    ...
    imp_supplier_contract.insert_issue(sess, vf, {"supply_ids": [1]})
    sess.commit()

    response = client.get(f"/e/supplier_contracts/{imp_supplier_contract.id}/issues")
    match(response, 200)
...
def test_supplier_rate_script_edit_get(client, sess):
    ...
    supplier_contract = supplier_party.insert_contract(
        sess,
        "Fusion Supplier 2000",
        "",
        {},
...
def test_supply_post(client, sess):
    valid_from = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    ...
    response = client.post(f"/e/supplies/{supply.id}", data=data)

    match(response, 200)

    sess.rollback()
    era = supply.find_era_at(sess, to_utc(ct_datetime(2010, 1, 1)))

    assert era.msn == new_msn


def test_supply_post_no_era_error(client, sess):
    valid_from = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    ...
    sess.rollback()
    era = supply.find_era_at(sess, to_utc(ct_datetime(2020, 1, 1)))
...
def test_supply_get_bill(sess, client):
    vf = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "22488", "Water Works")
    insert_sources(sess)
    source = Source.get_by_code(sess, "grid")
...
def test_supply_issues_get(client, sess):
    vf = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    ...
    response = client.get(f"/e/supplies/{supply.id}/issues")
...
def test_supply_months_get_export(sess, client):
    vf = to_utc(ct_datetime(1996, 1, 1))
    site = Site.insert(sess, "CI017", "Water Works")
    ...
    response = client.get(f"/e/supplies/{supply.id}/months", query_string=query_string)
    match(response, 200, r"Export\s*data by month")
