...
def test_SiteSource_init_hh_data(sess, mocker):
    ...
    dc_contract = dc_party.insert_contract(sess, "Fus DC", "", {}, vf, None, {})
    ...
    llfc = dno_party.insert_llfc(
        sess, "510", "HH HV", voltage_level, False, True, vf, None
    )
...
def test_SupplySource_init_nhh(sess, mocker):
    ...
    insert_voltage_levels(sess)
    voltage_level = VoltageLevel.get_by_code(sess, "HV")
    llfc_imp = dno_party.insert_llfc(
        sess,
        "510",
        "PC 5-8 & HH HV",
        voltage_level,
    ...
    llfc_exp = dno_party.insert_llfc(
        sess,
        "521",
        "Export (HV)",
        voltage_level,
...
"""
    ...
    start_date = to_utc(ct_datetime(2009, 8, 1, 0, 0))
    finish_date = to_utc(ct_datetime(2009, 8, 1, 23, 30))
    forecast_date = to_utc(ct_datetime(2020, 8, 1, 23, 30))
    caches = {}
    era = supply.eras[0]
