...
if mdd_ver is None:
    sys.exit(
        "Can't find a file beginning with " + GSP_PREFIX + " in the '" +
...
for nm in os.listdir(DOWNLOAD_DIR):
    shutil.copyfile(
        "download/" + nm,
        "original/" + nm[:-5-len(mdd_ver)] + ".csv")

dno_lookup = {}


gsp_group_map = {}
with open("original/GSP_Group.csv") as fl, \
        open("converted/gsp_group.csv", "w") as conv:
    f = csv.reader(fl)
    next(f)
...
participant_map = {}
with open("original/Market_Participant.csv") as fl, \
        open("converted/participant.csv", "w") as conv:
    f = csv.reader(fl)
    next(f)
    converted = csv.writer(conv)
...
market_role_map = {}
with open("original/Market_Role.csv") as fl, \
        open("converted/market_role.csv", "w") as conv:
    f = csv.reader(fl)
    next(f)
    converted = csv.writer(conv)
    converted.writerow(['id', 'code', 'description'])
...
meter_type_map = {}
with open("original/MTC_Meter_Type.csv") as fl, \
        open("converted/meter_type.csv", "w") as conv:
    f = csv.reader(fl)
    next(f)
    converted = csv.writer(conv)
...
with open("original/MTC_Payment_Type.csv") as fl, \
        open("converted/meter_payment_type.csv", "w") as conv:
    f = csv.reader(fl)
    next(f)
    converted = csv.writer(conv)
    converted.writerow(['id', 'code', 'description', 'valid_from', 'valid_to'])
...
pc_map = {}
with open("original/Profile_Class.csv") as fl, \
        open("converted/pc.csv", "w") as conv:
    eid = 0
    f = csv.reader(fl)
    next(f)
    converted = csv.writer(conv)
    ...
    converted.writerow(
        [eid, "00", "Half-hourly", '2000-01-01 00:00:00+00', ''])
    for line in f:
        eid += 1
        code = line[0].zfill(2)
        valid_from = line[1]
...
ssc_map = {}
with open("original/Standard_Settlement_Configuration.csv") as fl, \
        open("converted/ssc.csv", "w") as conv:
    f = csv.reader(fl)
    next(f)
    converted = csv.writer(conv)
    converted.writerow(
...
with open("original/Time_Pattern_Regime.csv") as fl, \
        open("converted/tpr.csv", "w") as conv:
    f = csv.reader(fl)
    next(f)
    converted = csv.writer(conv)
...
with open("original/Market_Participant_Role.csv") as fl, \
        open("converted/party.csv", "w") as conv:
    f = csv.reader(fl)
    next(f)
    converted = csv.writer(conv)
    converted.writerow(
        [
            'id', 'participant_id', 'market_role_id', 'name', 'valid_from',
            'valid_to', 'dno_code'])
...
with open("original/Clock_Interval.csv") as fl, \
    ...
    converted.writerow(
        [
            "id", "tpr_id", "Day of the Week Id", "Start Day",
            "Start Month", "End Day", "End Month", "Start Hour",
    ...
    for eid, fields in enumerate(f):
        ...
        converted.writerow(
            [eid, tpr_map[tpr_code]] + fields[1:-2] + time_fields)
...
with open("original/Line_Loss_Factor_Class.csv") as fl, \
        open("converted/llfc.csv", "w") as conv:
    f = csv.reader(fl)
    next(f)
...
with open("converted/mtc.csv", "w") as conv:
    ...
    converted.writerow(
        [
            'id', "dno_id", "code", "description", "has_related_metering",
    ...
    with open("original/Meter_Timeswitch_Class.csv") as orig_file:
        ...
        for fields in f:
            ...
            if 499 < mtc_code_int < 510 or 799 < mtc_code_int < 1000:
                ...
                meter_payment_type_id = meter_payment_type_map[
