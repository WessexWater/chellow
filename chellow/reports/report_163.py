...
from chellow.models import (
    Llfc, MarketRole, MeterType, Mtc, Participant, Party, Session,
    VoltageLevel
)
...
def _parse_Line_Loss_Factor_Class(sess, csv_reader):
    ...
    for values in csv_reader:
        ...
        if llfc is None:
            row = (
                'insert', 'llfc', dno.dno_code, llfc_code, description,
                voltage_level_code, is_substation, is_import,
                hh_format(valid_from), hh_format(valid_to, ongoing_str=''))
            rows.append(row)
            ...
            row = (
                ...
                hh_format(llfc.valid_from), description, voltage_level_code,
                is_substation, is_import, hh_format(valid_to, ongoing_str=''))
            rows.append(row)
...
def _parse_Market_Participant_Role(sess, csv_reader):
    ...
    for values in csv_reader:
        ...
        if party is None:
            row = (
                'insert', 'party', market_role_code, participant_code, name,
                hh_format(valid_from), hh_format(valid_to, ongoing_str=''),
                dno_code_str)
            rows.append(row)
            ...
            row = (
                'update', 'party', market_role_code, participant_code, name,
                hh_format(valid_from), hh_format(valid_to, ongoing_str=''),
                dno_code_str)
            rows.append(row)
        ...
        if is_common:
            ...
            if mtc is None:
                row = (
                    ...
                    has_related_metering, has_comms, is_hh, meter_type_code,
                    meter_payment_type_code, tpr_count, hh_format(valid_from),
        ...
        if mtc is None:
            ...
            rows.append(row)
                ...
                (
                    ...
                    valid_to != mtc.valid_to)):
            row = (
                'update', 'mtc', mtc.dno.dno_code, mtc.code, description,
                has_related_metering, has_comms, is_hh, meter_type_code,
                meter_payment_type_code, tpr_count, hh_format(mtc.valid_from),
                hh_format(valid_to, ongoing_str=''))
            rows.append(row)
...
def _parse_MTC_Meter_Type(sess, csv_reader):
    ...
    for values in csv_reader:
        ...
        pt = sess.query(MeterType).filter(
            MeterType.code == code, MeterType.valid_from == valid_from).first()
        if pt is None:
            row = (
