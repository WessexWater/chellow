...
from chellow.utils import (
    ct_datetime, ct_datetime_now, hh_format, req_int, to_utc
)
...
def content(user, file_name, file_like, dno_id):
    ...
    try:
        ...
        titles = (
            '# update', 'llfc', 'DNO Code', 'LLFC Code', 'Valid From',
            'LLFC Description', 'Voltage Level Code', 'Is Substation?',
            'Is Import?', 'Valid To'
        )
        ...
        for ss_llfc in ss_llfcs:
            ...
            llfc = sess.query(Llfc).filter(
                Llfc.dno == dno, Llfc.code == ss_llfc_code,
                Llfc.valid_from <= fy_finish, or_(
                    Llfc.valid_to == null(), Llfc.valid_to >= fy_start)
                ).first()
...
def _make_row(llfc, ss_llfc):
    if llfc.voltage_level.code != ss_llfc['voltage_level'] or \
            llfc.is_substation != ss_llfc['is_substation']:
        ...
        return (
            'update', 'llfc', llfc.dno.dno_code, llfc.code,
            hh_format(llfc.valid_from), '{no change}', new_vl, new_ss,
            '{no change}', '{no change}'
        )
