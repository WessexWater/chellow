...
def _process_supply(
    ...
    while len(bill_ids) > 0:
        ...
        bill = sess.query(Bill).filter(Bill.id == bill_id).options(
            joinedload(Bill.batch), joinedload(Bill.bill_type),
            joinedload(Bill.reads), joinedload(Bill.supply),
                ...
                try:
                    if typ != read_dict[key]:
                        virtual_bill['problem'] += " Reads taken " + \
                            "on " + str(dt) + " have differing read types."
                except KeyError:
                    read_dict[key] = typ
            ...
            for k, v in loads(covered_bill.breakdown).items():
                ...
                if isinstance(v, list):
                    ...
                    try:
                        ...
                        raise BadRequest(
                            "For key " + str(k) + " in " + str(
                                [b.id for b in covered_bills.values()]) +
                            " the value " + str(v) +
                    ...
                    if k.endswith('-gbp'):
                        ...
                        add_gap(
                            caches, gaps, elem, covered_bill.start_date,
        ...
        for era in sess.query(Era).filter(
                Era.supply == supply, Era.start_date <= covered_finish, or_(
                    Era.finish_date == null(),
                    Era.finish_date >= covered_start)
                ).distinct().options(
                joinedload(Era.channels), joinedload(Era.cop),
                joinedload(Era.dc_contract), joinedload(Era.exp_llfc),
            ...
            if contract not in (
                    ...
                    era.imp_supplier_contract, era.exp_supplier_contract):
            ...
            try:
                ds_key = (
                    chunk_start, chunk_finish, forecast_date, era.id, polarity,
        ...
        for start_date, hhgap in sorted(elgap.items()):
            if hhgap['has_virtual'] and not hhgap['has_covered']:
                ...
                if len(clumps) == 0 or not all(
                    ...
                    clumps.append(
                        {
                            'element': element, 'start_date': start_date,
                            'finish_date': start_date, 'gbp': hhgap['gbp']
                        })
                else:
    ...
    for i, clump in enumerate(clumps):
        ...
        vals['covered-problem'] = '_'.join(
            (
                ...
                'from', hh_format(clump['start_date'])))
