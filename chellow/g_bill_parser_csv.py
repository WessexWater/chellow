...
def get_datetime(row, idx, name, lineno):
    ...
    try:
        ...
        raise BadRequest(
            "Problem parsing the timestamp '" + val + "' at line number " +
            str(lineno) + " for the field '" + name + "' in column " +
            str(idx + 1) + ": " + e.description)
...
def get_decimal(row, idx, name, lineno):
    ...
    try:
        ...
        raise BadRequest(
            "Problem parsing the decimal '" + val + "' at line number " +
            str(lineno) + " for the field '" + name + "' at " + str(idx + 1) +
            ": " + str(e))
...
class Parser():
    ...
    def make_raw_bills(self):
        ...
        for self._line_number, row in enumerate(self.csv_reader):
            ...
            raw_bills.append(
                {
                    'mprn': mprn, 'reference': bill_reference,
                    'account': account, 'reads': reads, 'kwh': kwh,
                    'breakdown': breakdown, 'net_gbp': net, 'vat_gbp': vat,
                    'gross_gbp': gross,
