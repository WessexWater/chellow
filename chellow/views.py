...
def elexonportal(fname):
    ...
    return send_file(
        fname, mimetype=mimetype, as_attachment=True, attachment_filename=filename
    )
...
@views.route("/ecoes/NonDomesticCustomer/ExportPortfolioMPANs", methods=["GET"])
def ecoes_mpans_get():
    return Response(
        ",".join(
            (
                ...
                "Line Loss Factor",
                "Line Loss Factor Effective From Date",
                "Profile Class",
                ...
                "Data Collector",
                "Meter Operator",
                "Meter Operator Effective From Date",
                "GSP Group",
...
@views.route("/dc_contracts/<int:contract_id>/add_rate_script", methods=["POST"])
...
@views.route("/dc_rate_scripts/<int:dc_rate_script_id>/edit", methods=["POST"])
...
@views.route("/supplier_rate_scripts/<int:rate_script_id>/edit", methods=["POST"])
...
@views.route("/supplier_contracts/<int:contract_id>/add_rate_script", methods=["POST"])
...
@views.route("/mop_contracts/<int:contract_id>/add_rate_script", methods=["POST"])
...
limit :max_results"""
        ...
        if len(e_eras) == 1 and len(g_eras) == 0:
            return chellow_redirect("/supplies/" + str(e_eras[0].supply.id), 307)
        elif len(e_eras) == 0 and len(g_eras) == 1:
            return chellow_redirect("/g_supplies/" + str(g_eras[0].g_supply.id), 307)
        else:
            return render_template(
...
def channel_edit_post(channel_id):
    try:
        ...
        if "delete" in request.values:
            ...
            return chellow_redirect("/channels/" + str(channel_id) + "/edit", 303)
...
@views.route("/supplier_contracts/<int:contract_id>/add_batch", methods=["POST"])
...
def read_edit_post(read_id):
    try:
        ...
        if "update" in request.values:
            ...
            g.sess.commit()
            return chellow_redirect("/supplier_bills/" + str(read.bill.id), 303)
        elif "delete" in request.values:
            bill = read.bill
            read.delete(g.sess)
            g.sess.commit()
...
@views.route("/non_core_contracts/<int:contract_id>/add_rate_script", methods=["POST"])
...
def supplier_batch_edit_post(batch_id):
    try:
        ...
        if "update" in request.values:
            ...
            g.sess.commit()
            return chellow_redirect(
                "/supplier_batches?supplier_contract_id=" + str(contract_id), 303
            )
    except BadRequest as e:
        flash(e.description)
        return make_response(
...
@views.route("/supplies/<int:supply_id>/notes/<int:index>/edit", methods=["POST"])
def supply_note_edit_post(supply_id, index):
    try:
        ...
        if "delete" in request.values:
            ...
            g.sess.commit()
            return chellow_redirect("/supplies/" + str(supply_id) + "/notes", 303)
        else:
            category = req_str("category")
            is_important = req_bool("is_important")
            body = req_str("body")
...
@views.route("/non_core_contracts/<int:contract_id>/auto_importer", methods=["POST"])
...
@views.route("/industry_contracts/<contract_code>/rate_scripts/<start_date_str>")
...
@views.route("/energisation_statuses")
...
@views.route("/energisation_statuses/<int:energisation_status_id>")
...
@views.route("/g_contracts/<int:g_contract_id>/add_rate_script", methods=["POST"])
...
@views.route("/g_rate_scripts/<int:g_rate_script_id>/edit", methods=["POST"])
...
@views.route("/g_supplies/<int:g_supply_id>/notes/<int:index>/edit", methods=["POST"])
