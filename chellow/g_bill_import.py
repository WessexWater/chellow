...
def find_parser_names():
    return ", ".join(
        "." + name[14:].replace("_", ".")
        for module_finder, name, ispkg in iter_modules(chellow.__path__)
        if name.startswith("g_bill_parser_")
    )
...
class GBillImporter(threading.Thread):
    def __init__(self, sess, g_batch_id, file_name, file_bytes):
        ...
        if imp_mod is None:
            raise BadRequest(
                "Can't find a parser for the file '"
                + file_name
    ...
    def status(self):
        return (
            "Parsed file up to line number: "
            + str(self.parser.line_number)
            + ". Inserting raw bills up to bill: "
            + str(self.bill_num)
            + "."
        )
