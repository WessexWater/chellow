...
def reduce_bill_hhs(bill_hhs):
    ...
    for bill_hh in bill_hhs.values():
        ...
        try:
            bill_hh["net-gbp"] = sum(v["gbp"] for v in els_hh.values())
        except KeyError:
            raise BadRequest(f"Not all the elements have a 'gbp' in {els_hh}")
        for k, v in bill_hh.items():
            ...
            elif k.endswith("-gbp") and k != "net-gbp":
                raise BadRequest(f"Can't have a gbp outside elements bill[{k}]")
            else:
                if isinstance(v, set):
                    if k in bill:
                        bill[k].update(v)
