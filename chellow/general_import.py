...
def general_import_mop_bill(sess, action, vals, args):
    ...
    if action == "insert":
        ...
        mpan_core_str = add_arg(args, "Mpan Core", vals, 3)
        mpan_core = parse_mpan_core(mpan_core_str)
        supply = Supply.get_by_mpan_core(sess, mpan_core)
...
def general_import_g_bill(sess, action, vals, args):
    if action == "insert":
        ...
        finish_date_str = add_arg(args, "Finish Date", vals, 6)
        finish_date = parse_date(finish_date_str)
        net_gbp_str = add_arg(args, "Net GBP", vals, 7)
        net_gbp = Decimal(net_gbp_str)
        vat_gbp_str = add_arg(args, "Vat GBP", vals, 8)
        vat_gbp = Decimal(vat_gbp_str)
        gross_gbp_str = add_arg(args, "Gross GBP", vals, 9)
