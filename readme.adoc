...
Chellow comes with an upgrade script that checks PyPI and if there's a new
version, downloads and installs it. On Unix, set up a cron job by doing:

 crontab -e

and entering the line:

 * * * * * source /home/me/venv/bin/activate;chellow_updater.py
