...
public class GeneralImport extends Thread implements Urlable, XmlDescriber {
	...
	private void processItem(String[] values) throws HttpException {
		...
		if (type.equals("site")) {
			...
			if (action.equals("insert")) {
				...
				if (action.equals("delete")) {
					...
					csvElement.appendChild(getField("New Site Code", newCode));
			...
			if (action.equals("insert")) {
				...
				csvElement.appendChild(getField("Start date", startDateStr));
				String meterSerialNumber = values[6];
				csvElement.appendChild(getField("Meter Serial Number",
						meterSerialNumber));
				String importMpanStr = values[7];
				csvElement.appendChild(getField("Import MPAN", importMpanStr));
				String importSscCode = values[8];
				csvElement.appendChild(getField("Import SSC", importSscCode));
				...
				csvElement.appendChild(getField("Export MPAN", exportMpanStr));
				String exportSscCode = values[16];
				csvElement.appendChild(getField("Export SSC", exportSscCode));
				...
				site
						.insertSupply(supplyName, meterSerialNumber,
				...
				supply.update(supplyName.equals(NO_CHANGE) ? supply.getName()
						: supplyName, sourceCode.equals(NO_CHANGE) ? supply
			...
			if (action.equals("update")) {
				...
				csvElement.appendChild(getField("Finish date", finishDateStr));
				...
				if (importMpanStr != null) {
					...
					csvElement.appendChild(getField("Import is import kVArh",
					...
					csvElement.appendChild(getField("Import is export kVArh",
					...
					if (importHasImportKwh || importHasImportKvarh
						 ...
						 * importHhdce = Provider.getProvider( importHhdceStr,
						 * MarketRole.HHDC); }
						 */
					...
					csvElement.appendChild(getField("Import Supplier Contract",
				...
				csvElement.appendChild(getField("Eport MPAN", exportMpanStr));
				...
				if (exportMpanStr != null) {
					...
					csvElement.appendChild(getField("Export is import kVArh",
					...
					csvElement.appendChild(getField("Export is export kVArh",
					...
					csvElement.appendChild(getField("Export Supplier Contract",
					...
					csvElement.appendChild(getField("Export Supplier Account",
							exportSupplierAccountReference));
			...
			csvElement.appendChild(getField("Reference", hhdcAccountReference));
			if (action.equals("insert")) {
		...
		/*
		 * } catch (HttpException e) { String message = e.getMessage(); if
		 * (message == null) { message = "There has been a problem: " +
		 * e.getStackTraceString(); } csvElement.appendChild(new
		 * MonadMessage(message).toXml(doc)); source.appendChild(csvElement);
		 * throw e; }
		 */
	}
